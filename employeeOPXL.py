...
def salarycalculator():
    ...
    if "EmpSalary" in fp.sheetnames:
        fpsal = fp["EmpSalary"]
    else:
        fpsal = fp.create_sheet("EmpSalary")
        fpsal['A1'] = "ID"
        fpsal['B1'] = "BASIC"
        fpsal['C1'] = "H.R.A"
        fpsal['D1'] = "D.A"
    ...
    for i in fpws.iter_rows(values_only=True):
        if i[0] == 'ID':
            ...
            HRA = int(sal*(0.2))
            DA = int(sal*(0.6))
            INTX = int((sal + HRA + DA)*0.15)
            fpsal.cell(row,1).value = id
            fpsal.cell(row,2).value = sal
            fpsal.cell(row,3).value = HRA
            fpsal.cell(row,4).value = DA
            fpsal.cell(row,5).value = INTX
            fpsal.cell(row,6).value = (sal + HRA + DA)-INTX
        row += 1
...
Net Salary      --  ₹{netsal}""")
    ...
    if c == '1':
        ...
        # Image
        img = Image(r"C:\Users\karti\OneDrive\Pictures\Screenshots\Screenshot 2026-05-01 090143.png", width=120, height=110)
        content.append(img)
